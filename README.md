...
- 📄 **Control Anti-Desborde y Paginación Limpia**:
  ...
  - Habilita la división fluida de tablas entre páginas (cantSplit = 0).
...
- **set_cell_background(cell, hex_color)**: Inyecta elementos w:shd para colorear fondos de celdas y bloques.
...
- **dd_bottom_border(paragraph, hex_color, size)**: Crea líneas divisorias inferiores con w:pBdr.
- **dd_hyperlink(paragraph, url, text)**: Genera relaciones externas de tipo hipervínculo en el paquete OPC de Word.
- **llow_row_to_break(row)**: Configura w:cantSplit = 0 para permitir que el contenido fluya entre páginas sin cortar tablas.
